...
def choice_sort(A):
    ...
    Асимптотика O(N^). Устойчивая. Инвариант: все, что до i отсортировано.
    Берем элемент и смотрим есть ли справа меньшие, если есть, меняем и так  пробегаем по всем'''
